...
function calculerNotesAjax() {
    ...
    .then(data => {
        if (data.success) {
            ...
            showMessage('success', `${data.nbMisAJour} candidat(s) de l'année ${anneeSelectionnee} mis à jour avec succès`);
            
            // Mettre à jour les notes dans le tableau
            updateNotesInTable(data.notes);
        } else {
...
function updateNotesInTable(notes) {
    ...
    rows.forEach(row => {
        ...
        if (cells.length > 0) {
            ...
            // Si on a la note pour ce candidat
            if (notes[numCandidat] !== undefined) {
                // Trouver la cellule noteDossier
